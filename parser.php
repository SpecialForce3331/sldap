...
//----------------------------Получаем дату последнего обновления траффика--------------
...
//---------------------------закончили-------------------------------------
	...
	echo "start parse the logfile $pathLog\n";
//--------------------------------Парсим лог --------------------------------
	...
	while( ( $temp = fgets( $handler, 4096 ) ) !== false )
	{
		...
		{
            if( array_key_exists( 7, $parse ) )
            {
                if ( strcasecmp($parse[7], $logins[$i][0]) == 0 &&  $parse[0] > $lastDate  )
                {
...
//--------------------------------Обработка данных--------------------------------
    ...
    $resultArray = $mysqli->query( $query ) or die( $mysqli->error."\n can't get traffic\n");
    while( $result = $resultArray->fetch_array() )
    {
        $lastUpdate = $result[0];
        $currentTraffic = $result[1];
        $login = $result[2];

        $query = $mysqli->query("SELECT SUM(bytes) FROM usersTraffic WHERE dateTime > \"".$lastUpdate."\" AND login =\"".$login."\"");
        $result = $query->fetch_row();
        $traffic = round( $result[0]/1048576, 2 ) + $currentTraffic;

        if ( $traffic != 0 )
        {
            $query = $mysqli->query( "SELECT dateTime FROM usersTraffic WHERE login=\"".$login."\" ORDER BY id DESC LIMIT 1" );
            $result = $query->fetch_row();
            $currentUpdate = $result[0];

            $mysqli->query("UPDATE users SET trafficForDay=\"".$traffic."\", lastUpdate= \"".$currentUpdate."\" WHERE login =\"".$login."\"");
        }
    }
